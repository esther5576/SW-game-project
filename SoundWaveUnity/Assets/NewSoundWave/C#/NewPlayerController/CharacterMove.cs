...
public class CharacterMove : MonoBehaviour
{
	...
	void LateUpdate ()
	{
		...
		//Aiming system
		if (cameraContainer.GetComponent<CameraSysthemFreeCameraLoopEstherV2> ().tiltAngle <= -9f) {
			aim = true;
		} else {
			aim = false;
		}
		...
		if (aim == false) {
			...
			cameraContainer.GetComponent<CameraSysthemFreeCameraLoopEstherV2> ().tiltMin = 9;
			_mesh.GetComponent<MeshRenderer> ().enabled = true;
		} else {
			cameraContainer.GetComponent<CameraSysthemFreeCameraLoopEstherV2> ().tiltMin = 90;
